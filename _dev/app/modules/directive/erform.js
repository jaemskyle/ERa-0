angular.module('ERChart').directive('erform', function($state, constants, $timeout, cache, $log, $stateParams, $ionicPopup, $q, erutils, $document) {
    ...
    var showPatientIdPopupAndPrint = function(){
    ...
    var initSaveChart = function(){
      ...
      if ($state.is('root.form')){
        console.log('---state.is root.form: creating new chart---')
        checkChart().then(function(chartStatus){
          console.log('---chartStatus:valid > moving on..---')
          initNewChart(chartStatus);
        }, function(reason){
          console.log('---chartStatus:invalid > moving on..---')
          initNewChart(reason)
        });
      } else {
        console.log('---state.is root.edit: processing new form---')
    ...
    var initPrint = function(){
      ...
      ionic.Platform.ready(function(){
        cordova.plugins.printer.print(printPage, 'er-pid-' + $scope.chart.id || 'print', function () {
    ...
    $scope.$on('PageEvent:Print', function(event, args){
     // console.log('Event@erForm:: PageEvent:Print')
      checkChart().then(function(chart){
        initPrint();
        // document.addEventListener("deviceready", initPrint, false);
        },function(checkFailData){
            showPatientIdPopupAndPrint().then(function(popupResult){
              if (!popupResult){
              } else {
                initPrint();
                // document.addEventListener("deviceready", initPrint, false);
